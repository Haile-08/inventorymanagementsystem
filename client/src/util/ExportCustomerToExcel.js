...
const ExportCustomerToExcel = () => {
  const exportToExcel = (data) => {
    let sheetName = "invocatoryCustomerExport.xlsx";
    let headerName = "CustomerList";
    ...
    sheet.addTable({
      ...
      columns: [{ name: "Customer Report" }, { name: "Customer" }],
  ...
  const order = useSelector((state) => state.auth.customer);
  if (order.length === 0) {
...
export default ExportCustomerToExcel;
